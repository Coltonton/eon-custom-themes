...
- If those two avenues are not an option tell them to clone the main repo by SSHing into their EON and running `cd /data && git clone https://github.com/Coltonton/eon-custom-themes.git` then `exec /data/eon-custom-themes/restore_backup.py` after it completes selecting the 'Comma-Default' theme and choosing what all they want to install.

If you have any issues, questions, or what you have it please feel free to hit me up on discord! I’m in all the groups (except the main Comma.ai Server... Banned for nothing!) @Coltonton#2169! Hell, I might just be bored enough to do all this for you! Feel free to check out the [main community discord server](http://discord.gg/4UKcsmGJDq), and find my own little chat #custom-themes for more support from other then just me!
