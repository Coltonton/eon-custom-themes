...
int spin(int argc, char** argv) {
  ...
  for (int cnt = 0; ; cnt++) {
    ...
    if (draw_progress){
      ...
      //nvgFillColor(vg, nvgHSLA(color_hue / 360., .80, .57, 255));      // Add Text Code from @ShaneSmiskol
      //nvgTextAlign(vg, NVG_ALIGN_CENTER | NVG_ALIGN_TOP);
      //nvgFontSize(vg, 94.0f);
      //nvgText(vg, fb_w/2, (fb_h*4/5), "Loading OpenPilot...", NULL);
    } else {
      // message
      nvgTextAlign(vg, NVG_ALIGN_CENTER | NVG_ALIGN_TOP);
